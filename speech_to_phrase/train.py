...
async def train(
    ...
    elif model.type == ModelType.VIETASR:
        _LOGGER.debug("Skipping training for VietASR model %s", model.id)
    elif model.type == ModelType.CHUNKFORMER:
        _LOGGER.debug("Skipping training for ChunkFormer model %s", model.id)
    else:
